...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(
        colorScheme:
            ColorScheme.fromSeed(seedColor:const Color.fromARGB(255, 1, 26, 255)),
        useMaterial3: true,
      ),
                        ...
                        children: [
                          ...
                          Align(
                            ...
                            child: ElevatedButton(
                              onPressed: () {
                                String courseTitle = allModules[index]["title"];
                                ...
                                Navigator.push(
                                    ...
                                    MaterialPageRoute(
                                      builder: (context) => CourseDetailsPage(
                                        title: courseTitle,
                                        dip_id: allModules[index]["diploma_id"].toString(),
                                        mod_id:allModules[index]["id"].toString()),
                                    ));
                              },
...
class CourseDetailsPage extends StatefulWidget {
  // ignore: non_constant_identifier_names
  const CourseDetailsPage({super.key,required this.title,required this.dip_id,required this.mod_id});
        ...
        builder: (BuildContext context, AsyncSnapshot snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            if (allSubModules.isEmpty) {
              return const Center(
                child: Center(
                  child: Text("No data available. We are working to update more data."),
                ),
              );
            }
            return Center(
                        ...
                        children: [
                          ...
                          Align(
                            ...
                            child: ElevatedButton(
                              onPressed: () {
                                print(allSubModules);
                                String subModuletitle = allSubModules[index]["title"];
                                String subModuleDipID = allSubModules[index]["diploma_id"].toString();
                                String subModuleModID = allSubModules[index]["module_id"].toString();
                                String subModuleSubModID = allSubModules[index]["id"].toString();
                                // You can use this courseTitle to make the API request
                                Navigator.push(context, MaterialPageRoute(
                                  builder: (context) => CourseNotesPage(title:subModuletitle,dip_id: subModuleDipID,mod_id:subModuleModID,sub_mod_id:subModuleSubModID),
                                ));
                              },
...
class CourseNotesPage extends StatefulWidget{
  // ignore: non_constant_identifier_names
  const CourseNotesPage({super.key,required this.title,required this.dip_id,required this.mod_id, required this.sub_mod_id});
                          ...
                          Align(
                            ...
                            child:  ElevatedButton(
                                ...
                                // take data~ path and name
                                  Navigator.push(context, MaterialPageRoute(
                                    builder: (context) =>  PdfViewerPage(filePath:allNotes[index]["notes_pdf"],fileName: allNotes[index]["title"])
                                  ));
                              },
